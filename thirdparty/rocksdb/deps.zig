...
pub fn lib(
    ...
    const lib_rocksdb = b.addStaticLibrary(.{
        .name = "rocksdb",
        .root_source_file = b.path("thirdparty/rocksdb/root.zig"),
        .target = target,
        .optimize = optimize,
    });

    lib_rocksdb.linkLibC();
    lib_rocksdb.linkLibCpp();
    lib_rocksdb.addIncludePath(dep_rocksdb.path(""));
    lib_rocksdb.addIncludePath(dep_rocksdb.path("include"));

    lib_rocksdb.addCSourceFiles(.{
        .files = &rocksdb_srcs,
    ...
    if (target.result.os.tag == .linux) {
        const linux_liburing = @import("../linux_liburing/deps.zig").lib(b, target, optimize);

        lib_rocksdb.linkLibrary(linux_liburing);

        lib_rocksdb.root_module.include_dirs.appendSlice(
            b.allocator,
            linux_liburing.root_module.include_dirs.items,
        ) catch @panic("OOM");
    }
...
pub fn ldb(
    ...
    target: std.Build.ResolvedTarget,
    optimize: std.builtin.OptimizeMode,
    lib_rocksdb: *std.Build.Step.Compile,
) *std.Build.Step.Compile {
    const dep_rocksdb = b.dependency("rocksdb", .{});

    const exe_ldb = b.addExecutable(.{
        .name = "ldb",
        .target = target,
        .optimize = optimize,
    });
    ...
    exe_ldb.linkLibCpp();
    exe_ldb.linkLibrary(lib_rocksdb);
    exe_ldb.addIncludePath(dep_rocksdb.path(""));
    exe_ldb.addIncludePath(dep_rocksdb.path("include"));

    exe_ldb.addCSourceFiles(.{
        .files = &ldb_srcs,
        .root = dep_rocksdb.path(""),
        .flags = &cflags,
    });
